<template>
  <div class="col-12 payment_list">
    <div class="col-12 payment_list_info_con">
      <span>{{ registDateStr }}. {{ weekDay }}
        <span class="payment_status">
          <span class="bg_warning">미결제</span>
        </span>
        <span v-switch="status" class="payment_status">
          <span v-case="0" class="bg_warning">미결제</span>
          <span v-case="1" class="wait">결제대기</span>
          <span v-case="1" class="point0">결제완료</span>
        </span>
      </span>
    ...
    <a v-if="status == 0" href="javascript:void(0)" class="col-0 open_payment_btn" @click="openPaymentPopup">
      <span>결제하기</span>
    </a>
